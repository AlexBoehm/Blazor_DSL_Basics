            ...
            div(
                ...
                div(
                    input(
                        attrs(
                            type("text"),
                            bind.input.@string(this, filterText, value => filterText = value)
                        )
                    )
                ),
                ...
                ul(
                    from item in _items
                    where item.Text.Contains(filterText, StringComparison.OrdinalIgnoreCase)
                    select li(
                        input(
                            type("checkbox"),
                            bind.change.@checked(
                        ...
                        button(
                            attrs(
                                className("Delete"),
                                onClick(this, _ => _items.Remove(item))
                            ),
                            text("x")
                        )
                    )
